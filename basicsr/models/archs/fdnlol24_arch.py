...
class fourier_multi_scale_gamma2(nn.Module):
    def __init__(self,use_ratio):
        super(fourier_multi_scale_gamma2, self).__init__()
...
class MAR(nn.Module):
    def __init__(self,use_ratio=True):
        ...
        self.net=fourier_multi_scale_gamma2(use_ratio=use_ratio)
...
class FDSA(nn.Module):
    ...
    def forward(self, x):
        ...
        # qgg = torch.fft.fft2(q.float(), norm='backward')
        # kgg = torch.fft.fft2(k.float(), norm='backward')
        # _,_,h,w=qgg.shape
...
class Se(nn.Module):
    def __init__(self, in_channel, inner):
        super(Se, self).__init__()
        self.pool = nn.AdaptiveAvgPool2d(output_size=1)
        self.fc = nn.Sequential(
            nn.Linear(in_features=in_channel, out_features=inner, bias=False),
            nn.ReLU(),
            nn.Linear(in_features=inner, out_features=in_channel, bias=False),
            nn.Sigmoid()
        )
...
class PPM(nn.Module):
    def __init__(self, dim, bins=(1, 2, 3, 6)):
        ...
        self.fuse = nn.Sequential(
            nn.Conv2d(dim + reduction_dim * 4, dim, kernel_size=3, padding=1, bias=False, groups=dim // 2),
            nn.GELU())
...
class Img_merge(nn.Module):
    def __init__(self):
        super(Img_merge, self).__init__()
        self.s1 = SpatialAttention()
        self.s2 = SpatialAttention()

    def forward(self, x1, x2):
        return x1 * self.s1(x1) + x2 * self.s2(x2)


class FDN_lolv1(nn.Module):
    def __init__(self):
        super(FDN_lolv1, self).__init__()
        # self.net_a=former(inp_channels=3,
        #             out_channels=3,
        #               dim=32,
        #               num_blocks=[4,4,6],
        #               num_refinement_blocks=4,
        ...
        state = torch.load(
            '/data/tuluwei/code/restormer/fdn_lol/mar_lol.pth')
    ...
    def forward(self, inp_img, ori=None, device=None, ratio_i=None, mode=1):
        ...
        # global global_npy
        return inp_img,inp_img,inp_img,inp_img
