...
class EdgeLoss1(nn.Module):
    def __init__(self, loss_weight=1.0, reduction='mean'):
        super(EdgeLoss1, self).__init__()
...
class L_exp_gai(nn.Module):

    def __init__(self, patch_size=16):
        super(L_exp_gai, self).__init__()
        # print(1)
    ...
    def forward(self, x, gt):
        ...
        d = self.l1loss(mean_val, mean)
        return d


class L_exp_dowsample(nn.Module):
    def __init__(self, scale=1 / 8):
        super(L_exp_dowsample, self).__init__()
        # print(1)
        self.pool = nn.Upsample(scale_factor=scale, mode='bilinear', align_corners=False)
        self.l1loss = nn.L1Loss()
...
class dce_loss(nn.Module):
    def __init__(self, device):
        ...
        self.l_exp = L_exp(patch_size=16)
        self.l_spa = L_spa()
        self.l_tv = L_TV()
        self.l_ege = EdgeLoss(device)

    def forward(self, x, y, ori):
        l = 10 * torch.mean(self.l_exp(x, y)) + 200 * self.l_tv(x) + torch.mean(self.l_spa(x, ori))
        # l = torch.mean(self.l_exp(x, y))
        return l


class gamma_loss_2(nn.Module):
    def __init__(self, device):
        super(gamma_loss_2, self).__init__()
        self.l_exp1 = L_exp(patch_size=8)
        self.l_exp2 = L_exp(patch_size=16)
        self.l_tv = L_TV()
        # self.l_per=PerceptualLoss(layer_weights={'conv1_2': 1,'conv2_2': 1,'conv3_4': 1,'conv4_4': 1},
        #                      use_input_norm=True,perceptual_weight= 0.01,style_weight=0,range_norm= True,criterion='l1')

    def forward(self, x, y, ori=None):
        l = torch.mean(self.l_exp1(x, y)) + torch.mean(self.l_exp2(x, y)) + 20.0 * self.l_tv(x)
...
class a_loss(nn.Module):
    def __init__(self, device):
        super(a_loss, self).__init__()
        self.l_exp1 = L_exp_gai(patch_size=8)
        self.l_exp2 = L_exp_gai(patch_size=16)

    def forward(self, x, y, ori=None):
        l = self.l_exp1(x, y) + self.l_exp2(x, y) * 0.5
...
class gamma_loss(nn.Module):
    def __init__(self, device):
        ...
        self.l_exp1 = L_exp_dowsample(1 / 8)
        self.l_exp2 = L_exp_dowsample(1 / 16)


    def forward(self, x, y, ori=None):
        l = self.l_exp1(x, y) + self.l_exp2(x, y)
...
class MARLoss(nn.Module):
    def __init__(self, scale=1/8):
        ...
        self.down=nn.Upsample(scale_factor=scale, mode='bilinear', align_corners=False)
    def forward(self, x, y,vgg_loss):
        y_d=self.down(y)
        x_a=torch.abs(torch.fft.rfft2(x.float(), norm='backward'))
        y_a=torch.abs(torch.fft.rfft2(y_d.float(), norm='backward'))
        l = self.mseloss(x, y_d)+10.0*vgg_loss(x, y_d)[0]+0.01*self.mseloss(x_a,y_a)
